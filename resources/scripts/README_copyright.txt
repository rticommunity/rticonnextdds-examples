...
The second one is a flag to copy the copyright header in the corresponding files
or just check if that files have it or not. The other arguments are the 
extensions of the files that you want to examine (you may write many extensions 
as you want).
...
definition, so the XML files will look like:
  ...
  2. <option_flag> If the copyright will be checked, copied or deleted. 
        - A value '0' indicates that the copyright only will be checked if the 
          ...
          not have copyright, the script will report you an error.
  ...
  In order to know if some files have copyright (just checking for it), we can 
...
have copyright in the directory (and subdirectories): ./examples.
...
"external_examples.txt" in the next path:
  ...
  The written directories in this file will be skipped by the copyright.pl 
script. This file should contain the directories without finishing in "/" or 
"\". For example, the next line in external_examples.txt will skip the batching 
example in all the languages:
      ./examples/batching
Also, you can skip a file instead of a folder, you should write something like:
