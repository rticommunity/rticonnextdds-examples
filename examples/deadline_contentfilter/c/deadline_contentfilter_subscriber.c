    ...
    const struct DDS_RequestedDeadlineMissedStatus *status)
{
    ...
    DDS_ReturnCode_t retcode = deadline_contentfilterDataReader_get_key_value(
        deadline_contentfilterDataReader_narrow(reader), &dummy, &status->last_instance_handle);
    ...
    DDS_DataReader* reader)
{
    ...
    deadline_contentfilter_reader = deadline_contentfilterDataReader_narrow(reader);
...
static int subscriber_main(int domainId, int sample_count)
{
    ...
    struct DDS_Duration_t deadline_period = {2, 0}; // 2sec*/
    ...
    retcode = deadline_contentfilterTypeSupport_register_type(participant, type_name);
    if (retcode != DDS_RETCODE_OK) {
    ...
    for (count=0; (sample_count == 0) || (count < sample_count); ++count) {
        if(count == 10) {
            ...
            *DDS_StringSeq_get_reference(&parameters, 0) = DDS_String_dup("1");
            ...
            DDS_ContentFilteredTopic_set_expression_parameters(cft, &parameters);
        }
