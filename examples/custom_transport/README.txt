...
and FileTransport.h
The accompanying makefile_filetransport.x64Darwin10gcc411.mk can be used to build
the dynamic library in MacOSX for the x64Darwin10gcc411 platform.  
...
The custom_transport* files contain the auto-generated example applications created 
running rtiddsgen on the custom_transport.idl file. The publisher application was 
modified to change the value sent each cycle. The  subscriber is unchanged and
just prints the values received.

When custom_transport_publisher and custom_transport_subscriber are run they 
read the USER_QOS_PROFILE.xml file
