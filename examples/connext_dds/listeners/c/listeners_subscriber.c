        ...
        DDS_Subscriber *sub)
{
    ...
    if (++count > 3) {
        ...
        DDS_StatusMask newmask = DDS_REQUESTED_DEADLINE_MISSED_STATUS
...
static int subscriber_main(int domainId, int sample_count)
{
    ...
    participant_listener.as_subscriberlistener.as_datareaderlistener
            .on_sample_rejected = ParticipantListener_on_sample_rejected;
    participant_listener.as_subscriberlistener.as_datareaderlistener
            .on_liveliness_changed = ParticipantListener_on_liveliness_changed;
    participant_listener.as_subscriberlistener.as_datareaderlistener
            .on_sample_lost = ParticipantListener_on_sample_lost;
    participant_listener.as_subscriberlistener.as_datareaderlistener
            .on_subscription_matched =
            ParticipantListener_on_subscription_matched;
    participant_listener.as_subscriberlistener.as_datareaderlistener
            .on_data_available = ParticipantListener_on_data_available;
    ...
    participant_listener.as_subscriberlistener.as_datareaderlistener.as_listener
            .listener_data = &participant_listener;

    mask = DDS_REQUESTED_DEADLINE_MISSED_STATUS
            | DDS_REQUESTED_INCOMPATIBLE_QOS_STATUS | DDS_SAMPLE_REJECTED_STATUS
            | DDS_LIVELINESS_CHANGED_STATUS | DDS_SAMPLE_LOST_STATUS
            | DDS_SUBSCRIPTION_MATCHED_STATUS | DDS_DATA_AVAILABLE_STATUS
            | DDS_DATA_ON_READERS_STATUS | DDS_INCONSISTENT_TOPIC_STATUS;
    ...
    reader_listener.on_sample_rejected = ReaderListener_on_sample_rejected;
    reader_listener.on_liveliness_changed =
            ReaderListener_on_liveliness_changed;
    reader_listener.on_sample_lost = ReaderListener_on_sample_lost;
    reader_listener.on_subscription_matched =
            ReaderListener_on_subscription_matched;
    reader_listener.on_data_available = ReaderListener_on_data_available;
    reader_listener.as_listener.listener_data = &reader_listener;


    /* Just listen for liveliness changed and data available,
     * since most specific listeners will get called */
    mask = DDS_LIVELINESS_CHANGED_STATUS | DDS_DATA_AVAILABLE_STATUS;
