...
```sh
rtiddsgen -language c# -example x64Win64VS2013 -ppDisable ordered_group.idl
```
...
```sh
rtiddsgen -language c# -example x64Win64VS2013 -express -ppDisable ordered_group.idc
```
...
overwritten : /some/path/ordered_group_subscriber.cs
WARN com.rti.ndds.nddsgen.emitters.FileEmitter File exists and will not be
overwritten : /some/path/ordered_group_publisher.cs
```
...
project *ordered_group_type-dotnet4.0.vcxproj* contains the types of the example
...
```sh
bin\<build_type>\ordered_group_publisher.exe  <domain_id> <samples_to_send>
bin\<build_type>\ordered_group_subscriber.exe <domain_id> <sleep_periods>
```
