...
```sh
rtiddsgen -language c# -example x64Win64VS2013 -ppDisable coherent.idl
```
...
```sh
rtiddsgen -language c# -example x64Win64VS2013 -express -ppDisable coherent.idc
```
...
overwritten : /some/path/coherent_subscriber.cs
WARN com.rti.ndds.nddsgen.emitters.FileEmitter File exists and will not be
overwritten : /some/path/coherent_publisher.cs
```
...
project *coherent_type-dotnet4.0.vcxproj* contains the types of the example and
it should be compiled with *Visual Studio C++*. The other two projects contain
the C# publisher and subscriber example and should be compiled with *Visual
Studio C#*.
...
```sh
bin\<build_type>\coherent_publisher.exe  <domain_id> <samples_to_send>
bin\<build_type>\coherent_subscriber.exe <domain_id> <sleep_periods>
```
