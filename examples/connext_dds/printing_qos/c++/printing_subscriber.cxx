...
int run_subscriber_application(unsigned int domain_id, unsigned int sample_count)
{
     ...
     * The DDS_DomainParticipantQos_to_string_w_params API prints all the QoS values for
     * the DomainParticipantQos object.
     */
    participant_qos.to_string(str, strSize, DDS_QOS_PRINT_ALL, printFormat);
    str = DDS_String_alloc(strSize);
    if (str == NULL) {
        return shutdown_participant(participant, "String allocation error", EXIT_FAILURE);
    }
    participant_qos.to_string(str, strSize, DDS_QOS_PRINT_ALL, printFormat);
