...
inline ApplicationArguments parse_arguments(int argc, char *argv[])
{
    ...
    while (arg_processing < argc) {
        if ((argc > arg_processing + 1)
            ...
            break;
        } else if (strcmp(argv[arg_processing], "--normal-topic") == 0) {
            use_cft = false;
            arg_processing += 1;
        } else {
    ...
    if (show_usage) {
        std::cout
                << "Usage:\n"
                   ...
                   "                               Range: 0-3 \n"
                   "                               Default: 1\n"
                   "    --normal-topic             (subscriber only) If this "
                   "argument\n"
                   "                               is given, it will use a "
                   "normal\n"
                   "                               Topic instead of a Content "
                   "Filtered\n"
                   "                               Topic."
                << std::endl;
    }
