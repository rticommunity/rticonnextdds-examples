...
void example0Listener_on_requested_deadline_missed(
    void* listener_data,
...
void example0Listener_on_requested_incompatible_qos(
    void* listener_data,
    DDS_DataReader* reader,
...
void example0Listener_on_sample_rejected(
    void* listener_data,
    DDS_DataReader* reader,
...
void example0Listener_on_liveliness_changed(
    void* listener_data,
    DDS_DataReader* reader,
...
void example0Listener_on_sample_lost(
    void* listener_data,
    DDS_DataReader* reader,
...
void example0Listener_on_subscription_matched(
        void* listener_data,
        DDS_DataReader* reader,
...
void example0Listener_on_data_available(
        void* listener_data,
        DDS_DataReader* reader)
{
    example0DataReader *example0_reader = NULL;
    struct example0Seq data_seq = DDS_SEQUENCE_INITIALIZER;
    struct DDS_SampleInfoSeq info_seq = DDS_SEQUENCE_INITIALIZER;
    DDS_ReturnCode_t retcode;
    int i;

    example0_reader = example0DataReader_narrow(reader);
    if (example0_reader == NULL) {
        fprintf(stderr, "DataReader narrow error\n");
        return;
    }

    retcode = example0DataReader_take(
            example0_reader,
            &data_seq,
            &info_seq,
    ...
    for (i = 0; i < example0Seq_get_length(&data_seq); ++i) {
        if (DDS_SampleInfoSeq_get_reference(&info_seq, i)->valid_data) {
            printf("Received data\n");
            example0TypeSupport_print_data(
                    example0Seq_get_reference(&data_seq, i));
        }
    }

    retcode = example0DataReader_return_loan(
            example0_reader,
            &data_seq, &info_seq);
...
int subscriber_main(int domainId, int sample_count)
{
    ...
    retcode = example0TypeSupport_register_type(participant, type_name);
    if (retcode != DDS_RETCODE_OK) {
    ...
    example0Listener_on_requested_deadline_missed;
    reader_listener.on_requested_incompatible_qos =
    example0Listener_on_requested_incompatible_qos;
    reader_listener.on_sample_rejected =
    example0Listener_on_sample_rejected;
    reader_listener.on_liveliness_changed =
    example0Listener_on_liveliness_changed;
    reader_listener.on_sample_lost =
    example0Listener_on_sample_lost;
    reader_listener.on_subscription_matched =
    example0Listener_on_subscription_matched;
    reader_listener.on_data_available =
    example0Listener_on_data_available;
