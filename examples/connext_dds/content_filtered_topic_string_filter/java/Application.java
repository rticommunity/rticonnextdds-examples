...
public abstract class Application {
    ...
    private boolean useCftTopic = true;

    public void parseArguments(
            String[] args,
            ApplicationType current_application)
    {
        ...
        while (argProcessing < args.length) {
            ...
            if ((args.length > argProcessing + 1)
                ...
                argProcessing++;
                    ...
                    current_application == ApplicationType.SUBSCRIBER
                    && argument.equals("--normal-topic")) {
                useCftTopic = false;
            } else if (
                    (args.length > argProcessing + 1)
                    && (argument.equals("-v")
        ...
        if (showUsage) {
            ...
            if (current_application == ApplicationType.SUBSCRIBER) {
                System.out.println(
                      "    --normal-topic             If this arguments is given, the" + NL
                    + "                               subscriber will use a normal topic" + NL
