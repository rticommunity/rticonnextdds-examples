...
A publication of data of type instance
...
Example publication of type instance automatically generated by
...
int publisher_main(int domainId, int sample_count)
{
    ...
    instanceDataWriter *instance_writer = NULL;
    ...
    instance_writer = instanceDataWriter_narrow(writer);
    if (instance_writer == NULL) {
    ...
    instance = instanceTypeSupport_create_data_ex(DDS_BOOLEAN_TRUE);
    if (instance == NULL) {
        fprintf(stderr, "instanceTypeSupport_create_data error\n");
        publisher_shutdown(participant);
        return -1;
    }
    ...
    for (count=0; (sample_count == 0) || (count < sample_count); ++count) {
        ...
        retcode = DDS_DataWriter_get_datawriter_cache_status(
                writer,
                &cache_status);
        if (retcode != DDS_RETCODE_OK) {
            fprintf(stderr, "get_datawriter_cache_status error\n");
            publisher_shutdown(participant);
            return -1;
        }
    ...
    /* Delete data sample */
    retcode = instanceTypeSupport_delete_data_ex(instance, DDS_BOOLEAN_TRUE);
    if (retcode != DDS_RETCODE_OK) {
        fprintf(stderr, "instanceTypeSupport_delete_data error %d\n", retcode);
    }
