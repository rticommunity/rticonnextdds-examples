...
This exposes two points. First, even though the builtin Topics correspond to a hierarchy of Participants -> DataWriters/DataReaders, there is no hierarchy of listener callbacks. If, as in this example, we listen for Participant and DataReader information, both listeners will get the on_data_available() callbacks. If we want to track relationships between discovered participants and DataReaders/DataWriters, we may do so by keys. 
