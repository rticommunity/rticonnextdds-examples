...
/*    retcode = DDS_DomainParticipant_get_default_publisher_qos(participant, &publisher_qos);
    if (retcode != DDS_RETCODE_OK) {
...
*/    /* To customize publisher QoS, use 
       the configuration file USER_QOS_PROFILES.xml */
/*    publisher = DDS_DomainParticipant_create_publisher(
        participant, &publisher_qos, NULL,
        DDS_STATUS_MASK_NONE);
    if (publisher == NULL) {
        printf("create_publisher error\n");
        publisher_shutdown(participant, &publisher_qos, &datawriter_qos);
...
*/    /* End changes for coherent_presentation */
    ...
    retcode = coherentTypeSupport_register_type(
...
*/    /* End changes for coherent_presentation */
    ...
    instance->id = 0;
    instance_handle = coherentDataWriter_register_instance(
        coherent_writer, instance);
    ...
    for (count=0; (sample_count == 0) || (count < sample_count); ++count) {
        ...
        retcode = coherentDataWriter_write(
            coherent_writer, instance, &instance_handle);
        if (retcode != DDS_RETCODE_OK) {
