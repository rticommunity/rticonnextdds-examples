...
void MarketDataListener_on_requested_deadline_missed(void* listener_data,
        DDS_DataReader* reader,
        const struct DDS_RequestedDeadlineMissedStatus *status) {
}

void MarketDataListener_on_requested_incompatible_qos(void* listener_data,
        DDS_DataReader* reader,
        const struct DDS_RequestedIncompatibleQosStatus *status) {
}

void MarketDataListener_on_sample_rejected(void* listener_data,
        DDS_DataReader* reader, const struct DDS_SampleRejectedStatus *status) {
}

void MarketDataListener_on_liveliness_changed(void* listener_data,
        DDS_DataReader* reader,
        const struct DDS_LivelinessChangedStatus *status) {
}

void MarketDataListener_on_sample_lost(void* listener_data,
        DDS_DataReader* reader, const struct DDS_SampleLostStatus *status) {
}

void MarketDataListener_on_subscription_matched(void* listener_data,
        DDS_DataReader* reader,
        const struct DDS_SubscriptionMatchedStatus *status) {
}

void MarketDataListener_on_data_available(void* listener_data,
        DDS_DataReader* reader) {
    MarketDataDataReader *MarketData_reader = NULL;
    struct MarketDataSeq data_seq = DDS_SEQUENCE_INITIALIZER;
    struct DDS_SampleInfoSeq info_seq = DDS_SEQUENCE_INITIALIZER;
    DDS_ReturnCode_t retcode;
    int i;

    MarketData_reader = MarketDataDataReader_narrow(reader);
    if (MarketData_reader == NULL) {
        printf("DataReader narrow error\n");
        return;
    }

    retcode = MarketDataDataReader_take(MarketData_reader, &data_seq, &info_seq,
    ...
    for (i = 0; i < MarketDataSeq_get_length(&data_seq); ++i) {
        if (DDS_SampleInfoSeq_get_reference(&info_seq, i)->valid_data) {
            MarketDataTypeSupport_print_data(
                    MarketDataSeq_get_reference(&data_seq, i));
        }
    }

    retcode = MarketDataDataReader_return_loan(MarketData_reader, &data_seq,
            &info_seq);
...
static int subscriber_main(int domainId, int sample_count) {
    ...
    filtered_topic =
            DDS_DomainParticipant_create_contentfilteredtopic_with_filter(
                    participant, "Example MarketData", topic,
    ...
    reader_listener.on_requested_deadline_missed =
            MarketDataListener_on_requested_deadline_missed;
    reader_listener.on_requested_incompatible_qos =
            MarketDataListener_on_requested_incompatible_qos;
    reader_listener.on_sample_rejected = MarketDataListener_on_sample_rejected;
    reader_listener.on_liveliness_changed =
            MarketDataListener_on_liveliness_changed;
    reader_listener.on_sample_lost = MarketDataListener_on_sample_lost;
    reader_listener.on_subscription_matched =
            MarketDataListener_on_subscription_matched;
    reader_listener.on_data_available = MarketDataListener_on_data_available;
    ...
    for (count = 0; (sample_count == 0) || (count < sample_count); ++count) {
        /* printf("MarketData subscriber sleeping for %d sec...\n",
         poll_period.sec);
         */
