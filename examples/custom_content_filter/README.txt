...
Concept
-------
Custom Content Filters are similar to the 'Content Filtered Topic' (see example
in the community portal). The main difference is that Custom Content Filters
allow the use of non-relational operations between topic members, i.e. a 
computation involving topic members. Similarly to what happens with the Content 
Filtered Topics, after applying a custom content filter you will get a subset of
all samples that fulfill an expression.
...
through if the parameter[0] divides or is greater than x, respectively. 
...
the current parameter and a pointer to the evaluation function. When evaluate() 
is called for each sample, it merely returns the result of that function applied
to the sample data. 
