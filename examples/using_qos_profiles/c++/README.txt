Applies to RTI Data Distribution Service 4.3 and above. The following example has been written in C++. C and Java versions will be posted at a later date.
...
PARTICIPANT:
As a parenthetical, the Transport Properties example in section 15.6.6 of the User's Manual is reproduced in its context in myReaderProfiles.xml, with the values replaced by the default values of 9216.
...
Using various API calls relating to defaults, we create two subscribers and three readers:
    ...
    * subscriber1: partition A
          o reader1: middleware default QoS (subscriber1->set_default_profile() by itself has no effect on reader1 if reader1 is created with DDS_DATAREADER_QOS_DEFAULT)
...
File C:\local\QoS_Profiles\c++\profiles_subscriber.cxx already exists and will not be replaced with updated content. If you would like to get a new file with the new content, either remove this file or supply -replace option.
File C:\local\QoS_Profiles\c++\profiles_publisher.cxx already exists and will not be replaced with updated content. If you would like to get a new file with the new content, either remove this file or supply -replace option.
